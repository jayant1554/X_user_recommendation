...
def _rank_recommendations(
    ...
    retrieved = app_state.generator.get_candidates(
        target,
        k=retrieved_k,
    )
    ...
    retrieved_response = [
        RetrievedCandidate(
            ...
            shared_interests=sorted(
                target.interest_set & candidate.user.interest_set
            ),
        )
    ...
    recommendation_response = [
        ...
        for index, (candidate, final_score) in enumerate(
            ranked[:top_k],
            start=1,
        )
    ]
